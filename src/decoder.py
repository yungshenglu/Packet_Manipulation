...
def main():
    ...
    with open('../../list.txt') as file:
        for line in file:
            temp = line[0 : -1] + line[0 : -1]
            temp = ''.join(reversed(temp[0 : 14]))
            keys.append(temp)
            lists.append(line[0 : -1])
            color.append(line[3 : 6])
    
    for i, val in enumerate(color):
        print(val)

    for index, student_id in enumerate(lists):
        my_file = Path('../../secret/' + student_id + '.txt')
        key = ''
        id = ''
        data = []
        if my_file.exists():
            print(my_file)
            with open('../../secret/' + student_id + '.txt') as file:
                for line in file:
                    key = key + line[0]
